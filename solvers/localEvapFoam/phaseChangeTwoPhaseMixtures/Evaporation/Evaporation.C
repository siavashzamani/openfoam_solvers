...
Foam::phaseChangeTwoPhaseMixtures::Evaporation::mDotAlphal() const
{
    ...
    const volScalarField& alpha_Press = alpha1().db().lookupObject<volScalarField>("alpha_Press");
    ...
    (
        ...
        -1.0*dimdummy*rhoGas*massFlux*alpha_Area*alpha_Press/((alpha_volume+SMALL)*(alpha1_dividable))
        //-1.0*dimdummy*massFlux*alpha_Area/((alpha_volume+SMALL)*(alpha1_dividable))
    );
}
